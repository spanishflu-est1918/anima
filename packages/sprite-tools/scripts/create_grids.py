...
def extract_frames(video_path: Path, output_dir: Path, crop_top: int = None, crop_bottom: int = None, auto_crop: bool = True) -> tuple[int, int, int, tuple[int, int]]:
    """Extract frames from video with optional letterbox crop.

    Returns: (frame_count, crop_top_used, crop_bottom_used, original_size)
    """
    ...
    return len(frames), crop_top, crop_bottom, original_size
...
def create_grids(frames_dir: Path, output_dir: Path,
    ...
    # Calculate restore padding (to get back to original size)
    if original_size and (crop_top > 0 or crop_bottom > 0):
        orig_w, orig_h = original_size
        total_cropped = crop_top + crop_bottom
        # Padding to restore: split cropped pixels between top/bottom
        restore_top = crop_top
        restore_bottom = crop_bottom
    else:
        restore_top = restore_bottom = 0
    ...
    meta = {
        "source": {
            ...
            "restore_padding": {
                "top": restore_top,
                "bottom": restore_bottom,
                "note": f"Add this padding to restore original {original_size[1] if original_size else 'N/A'}px height"
            } if crop_top > 0 or crop_bottom > 0 else None
        },
        "grid": {
            "size": grid_size,
            "cols": cols,
...
def main():
    ...
    total_frames, crop_top, crop_bottom, original_size = extract_frames(
